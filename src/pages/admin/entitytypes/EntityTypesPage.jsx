...
export default function EntityTypesPage() {
  ...
  const fetchTypes = useCallback((targetPage) => {
    ...
    setLoading(true);
    setError(null);
    entityTypes.list({ page: p, size: PAGE_SIZE })
      .then(res => {
        setItems(res.data);
        setTotal(res.total);
  ...
  const handleSave = async (payload) => {
    try {
      if (modal.type === 'create') {
        await entityTypes.create(payload);
      } else {
        await entityTypes.update(modal.item.id, payload);
      }
      setModal(null);
      fetchTypes(1);
    } catch (e) {
  ...
  return (
    <div className="crud-page">
      <div className="crud-header">
        <div>
          <h1 className="crud-title">Entity Types</h1>
      ...
      <div className="crud-table-wrap">
              ...
              {loading ? (
                ...
                <tr className="crud-row">
                  <td className="crud-state-row" colSpan={3}>
                    <p className="crud-error-text">Failed to load entity types</p>
                    <button onClick={() => { setPage(1); fetchTypes(); }} className="crud-retry-btn">Retry</button>
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <tr className="crud-row"><td className="crud-state-row" colSpan={3}>No entity types yet.</td></tr>
              ) : items.map((item, i) => (
        ...
        <Pagination page={page} totalPages={totalPages} total={total} pageSize={PAGE_SIZE} label="types" onPageChange={setPage} />
      </div>
      ...
      <div className="modal-container">
        <div className="modal-card" onClick={e => e.stopPropagation()}>
          <div className="modal-header">
            <h2 className="modal-title">{item ? 'Edit' : 'New'} Entity Type</h2>
          ...
          <form onSubmit={handleSubmit} className="modal-form">
            <div>
              ...
              <input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Person, Organization, Asset" className="form-input" />
            </div>
