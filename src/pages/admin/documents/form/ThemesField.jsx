...
export default function ThemesField({ rows, setRows }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center gap-3 field-input bg-[#f9fafc] cursor-default min-h-[42px]">
          {rows.length === 0 ? (
      ...
      {expanded && (
        <div className="slidein mt-3 border border-[#e2e6ef] rounded-xl overflow-hidden">
                ...
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => removeRow(r.id)}
                      className="w-7 h-7 rounded-md flex items-center justify-center text-[#c0392b] hover:bg-[#c0392b]/10 transition-colors">
          ...
          <div className="border-t border-slate-100 bg-[#f9fafc] p-3 flex items-center gap-2">
            ...
            <div className="relative flex-1">
              <select value={sub} onChange={e => setSub(e.target.value)} disabled={!theme}
            ...
            <button onClick={addRow} disabled={!theme || !sub}
              className={`px-4 py-2.5 rounded-lg text-[12px] font-bold tracking-wide shrink-0 transition-all
                ${theme && sub ? 'bg-[#c0392b] text-white hover:opacity-90' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}>
              ADD
            </button>
          </div>
        </div>
