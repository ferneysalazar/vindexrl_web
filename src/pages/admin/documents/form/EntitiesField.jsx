...
export default function EntitiesField({ entities, setEntities, disabled }) {
  ...
  const add    = () => {
    if (disabled) return;
    setEntities(prev => [...prev, { id: uid(), value: '' }]); setExpanded(true);
  };
  const remove = (id) => {
    if (disabled) return;
    setEntities(prev => (prev.length > 1 ? prev.filter(e => e.id !== id) : prev));
  };
      ...
      <div className={`flex flex-col gap-2 ${disabled ? 'opacity-50' : ''}`}>
        {entities.map((e, i) => (
          <div key={e.id} className="flex items-center gap-2">
            ...
            <button onClick={() => remove(e.id)} disabled={disabled || entities.length === 1}
              className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors
                ${disabled || entities.length === 1 ? 'text-slate-300 cursor-not-allowed' : 'text-[#c0392b] hover:bg-[#c0392b]/10'}`}>
